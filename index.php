...
function increaseQuantity(productId) {
    ...
    cartItemCount++; // Увеличиваем общее количество товаров
    total += cart[productId].price; // Увеличиваем общую сумму
    renderCart();
    updateForm();
    updateCartCount();
    updateTotal();
...
function decreaseQuantity(productId) {
    ...
    if (cart[productId].quantity > 1) {
        cart[productId].quantity--;
        cart[productId].total -= cart[productId].price;
        cartItemCount--; // Уменьшаем общее количество товаров
        total -= cart[productId].price; // Уменьшаем общую сумму
    } else {
    ...
    updateTotal();
        ...
        function updateTotal() {
            ...
            totalContainer.querySelector('.total').textContent = `$${total.toFixed(2)}`;
        }
        ...
        document.addEventListener('click', function(event) {
            if (event.target.classList.contains('remove-item')) {
                ...
                // Если количество товара стало нулевым, удаляем его из корзины
                if (cart[productId].quantity === 0) {
                    delete cart[productId];
                } else {
                    // Если остались еще товары этого типа, корректируем общую сумму
                    cart[productId].total -= cart[productId].price;
                }
